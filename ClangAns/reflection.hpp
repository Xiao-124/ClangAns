...
typedef std::tuple<setFuncion, getFuncion, getNameFuncion, getNameFuncion, getNameFuncion, getBoolFunc>
    filed_function_tuple;		//反射函数的一些定义。
...
namespace Reflection
{
    ...
    public:
        ...
        static void registerToArrayMap(const char* name, array_function_tuple* value);	//将 tuple添加至map

        static void unregisterAll();	//删除map中的所有tuple
    };
    ...
    public:
        ...
        ReflectionInstance& operator=(ReflectionInstance&& dest);
        ReflectionInstance(const ReflectionInstance& dest){
            this->m_instance = dest.m_instance;
            this->m_meta = dest.m_meta;
        }
